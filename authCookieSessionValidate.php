...
//else if (! empty($_COOKIE["member_login"]) && ! empty($_COOKIE["random_password"]) && ! empty($_COOKIE["random_selector"])) {
else if (! empty($_COOKIE["selector"]) && ! empty($_COOKIE["validator"])) {
   ...
   // }
    ...
    if ($isAuthValidatorVerified && $isAuthExpiryDateVerified ) {    
       $isLoggedIn = true;
       } 
    // Else, mark the token as expired and clear cookies   
        ...
        //}
        if(!empty($userAuthToken[0]["userid"])) {
            $auth->deleteAuthToken($userAuthToken[0]["useidid"]);
        }
        // clear cookies
